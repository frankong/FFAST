...
%% Set Parameters
D = 2;      % number of delays for each chain
C = 1;      % number of chains for delays
kay_flag = 1; % Use kay estimation
noise_flag = 0;
very_sparse = 0;
...
spectrum2 = spectrum2/max(abs(spectrum2(:)))*sqrt(rho);
...
figure,imshowf(abs(spectrum2),[0,sqrt(rho)*0.1]),title('Original Spectrum','FontSize',14),drawnow;
...
noisy_spectrum2 = spectrum2 + sigma_z*(randn([nx,ny])+1i*randn([nx,ny]))/sqrt(2);

signal2 = ifft2(noisy_spectrum2);


figure,imshowf(abs(noisy_spectrum2),[0,sqrt(rho)*0.1]),title('Noisy Spectrum','FontSize',14),drawnow;
